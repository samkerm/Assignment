...
open class Search {
                ...
                if let response = response, let searchData = response.data, let pagination = response.pagination, searchData.count != 0 {
                    ...
                    for result in results {
                        self.fetchDataFrom(URL: result.gifUrl, completion: { (data, error) in
                            ...
                            print("Fetching data number: ", fetchedDataCount)
    ...
    // In case the searched title didnt exist on GIPHY database
    ...
    // When we get JSON file we can parse it to our needed classs
    ...
    // URLs fetched need to be requested for their data
